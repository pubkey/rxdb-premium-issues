...
describe('bug-report.test.ts', () => {
    describe('SQLite bulkUpsert data corruption', () => {
        ...
        it('should correctly update documents via bulkUpsert (second pass)', async function() {
            ...
            await collection.bulkRemove(existingDocs);

            // Just for demonstration of the issue in detail,
            // otherwise only 75 documents are updated in the second pass, not all 15000
            ...
            // ==> Result only 75 Elements are upserted
            // Error log is empty
            const updateEnd = performance.now();
            console.log(`Update completed in ${(updateEnd - updateStart).toFixed(2)}ms`);
            await new Promise(resolve => setTimeout(resolve, 50000)); // Wait a moment for any async processing to complete


            // Verify count is unchanged (should be updates, not inserts)
        ...
        it('should return correct results for indexed field queries', async function() {
            ...
            // Note: After update, all documents should still have their original projectId
            assert.ok(
